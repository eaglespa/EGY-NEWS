...
export function Header({ lang }: { lang: Locale }) {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="border-b border-line bg-bg/80 backdrop-blur-2xl">
        <div className="container-x flex items-center gap-3 py-3">
          ...
          <nav className="ms-6 hidden items-center gap-1 lg:flex" aria-label={t.menu}>
            <Link
              href={`/${lang}`}
              className="rounded-full px-3.5 py-2 text-sm font-medium text-ink2 transition-colors hover:bg-panel hover:text-gold"
            >
              {t.home}
            </Link>
            {CATEGORIES.map((c) => (
          ...
          <div className="ms-auto flex items-center gap-2">
            ...
            <Link
              href={`/${lang}/advertise`}
              className="hidden items-center gap-2 rounded-full border border-gold/40 bg-gold/10 px-4 py-2 text-sm font-semibold text-gold transition-all hover:bg-gold hover:text-black md:flex"
            >
              {t.advertise}
            </Link>
            <ThemeToggle />
